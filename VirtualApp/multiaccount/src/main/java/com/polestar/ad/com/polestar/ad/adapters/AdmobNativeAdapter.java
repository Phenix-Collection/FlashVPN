...
public class AdmobNativeAdapter extends NativeAd implements INativeAdLoader {
    ...
    public void loadAd(int num, INativeAdLoadListener listener) {
        ...
        if (TextUtils.isEmpty(mFilter) || mFilter.equals(Constants.AdMob.FILTER_BOTH_INSTALL_AND_CONTENT)){
            isContent = true;
            isInstall = true;
        } else if (mFilter.equals(Constants.AdMob.FILTER_ONLY_CONTENT)){
            isContent = true;
        }else if (mFilter.equals(Constants.AdMob.FILTER_ONLY_INSTALL)) {
            isInstall = true;
        }
