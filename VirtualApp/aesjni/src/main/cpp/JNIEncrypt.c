...
//    }
    ...
    jstring key = charToJstring(env, (char*)AES_KEY);
    __android_log_write(ANDROID_LOG_INFO,"code",(char*)AES_KEY);
    const char *in = (*env)->GetStringUTFChars(env, str_, JNI_FALSE);
    //char *baseResult = AES_128_ECB_PKCS5Padding_Encrypt(in, (uint8_t*)"0123456789ABCDEF");
