...
//    }
    ...
    public static void reloadLockerSetting() {
        String key = PreferencesUtils.getEncodedPatternPassword(DualApp.getApp());
    ...
    public static boolean isAppLaunched(String pkg, int userId) {
        ...
        boolean hasSupportLib= DualApp.isSupportPkgExist();
        return (VirtualCore.get().isAppRunning(pkg, userId)
                || (hasSupportLib && new CloneAgent64(DualApp.getApp()).isAppRunning(pkg, userId)))
